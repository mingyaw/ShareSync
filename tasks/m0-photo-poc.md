...
- [x] Apply transfer limits after prioritizing already-downloaded photos.
...
Completed an iOS transfer-planning safety slice:

- Locked down single-photo and small-batch planning so transfer limits are applied after downloaded-photo priority.
- Ensured an already-downloaded photo is selected for import before failed retry or new download candidates when `limit` is small.
- Added Swift planner coverage for the small-limit validation path.
...
Completed an Android retry-flow regression slice:

- Added Android store coverage for a media result moving from `failed` to `synced` across batches.
- Added Android manifest coverage proving a retried-success photo is excluded from the next M0 manifest.
- Preserved failed media retry behavior while locking down the successful retry completion path.
...
Completed an Android M0 validation visibility slice:

- Added a local request counter to Android M0 request activity tracking.
- Updated the Android M0 screen to show the latest endpoint request number.
- Added router coverage for request counts across health, manifest, and media endpoints.
